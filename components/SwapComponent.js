...
const SwapComponent = () => {
	...
	useEffect(() => {
		...
		//Resseting isReversed value if it is set
	...
	async function handleSwap() {
		if (srcToken === ETH && destToken !== ETH) {
			performSwap(); // Self: (srcToken = ETH && destToken = ETH) is not taken care of
		} else {
	...
	async function handleReverseExchange(e) {
		// Setting the isReversed value to prevent input/output values
		// being calculated in their respective side - effects
		isReversed.current = true;

		//Checking the followimg situations
	...
	function getSwapBtnClassName() {
		let className =
			"p-4 w-full my-2 rounded-xl       text-zinc-400 bg-zinc-800 pointer-events-none";
		className +=
